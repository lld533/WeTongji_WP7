...
namespace WeTongji
{
    public partial class OfficialNote : PhoneApplicationPage
    {
        ...
        protected override void OnNavigatedTo(NavigationEventArgs e)
        {
            ...
            var uri = e.Uri.ToString().TrimStart("/Pages/OfficialNote.xaml".ToCharArray());
            ...
            WTDispatcher.Instance.Do(() =>
            {
                ...
                if (fs.IsIllustrated)
                {
                    ...
                    for (int i = 0; i < count; ++i)
                    {
                        ...
                        if (!img.Url.EndsWith("missing.png") && !fs.ImageExists(i))
                        {
                            var client = new WTDownloadImageClient();

                            client.DownloadImageStarted += (obj, arg) =>
                            {
                                Debug.WriteLine("Download NO.{0} image of Official Note[Id={1}] started: {2}",
                                    j, fs.Id, arg.Url);
                            ...
                            {
                                Debug.WriteLine("Download NO.{0} image of Official Note[Id={1}] FAILED: {2}.\nError:{3}",
                                    j, fs.Id, arg.Url, arg.Error);
                            ...
                            {
                                Debug.WriteLine("Download NO.{0} image of Official Note[Id={1}] completed: {2}",
                                    j, fs.Id, arg.Url);

                                fs.SaveImage(arg.ImageStream, j);
